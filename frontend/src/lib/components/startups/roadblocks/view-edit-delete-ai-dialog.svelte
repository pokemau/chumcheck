<script lang="ts">
  ...
  async function loadChatHistory() {
    ...
    try {
      const response = await fetch(
        `${PUBLIC_API_URL}/chat-history/roadblocks/${roadblock.id}`
      );
  ...
  async function handleSendMessage() {
    ...
    try {
      const response = await fetch(
        `${PUBLIC_API_URL}/roadblocks/${roadblock.id}/refine`,
        {
          method: 'POST',
          headers: {
          ...
          {:else}
            <div class="flex justify-start">
              <div class="rounded-lg bg-[#1e293b] p-4 text-white">
                I can help you refine this Roadblock. How would you like to
                    ...
                    <div class="rounded-lg bg-[#1e293b] p-4 text-white">
                      ...
                      {#if message.refinedDescription}
                        ...
                        >
                          ...
                          <button
                            class="absolute bottom-2 right-2 flex items-center gap-1 text-sm text-gray-400 opacity-50 transition-opacity hover:opacity-100"
                            onclick={(e) => {
                              const button =
            ...
            <div class="mb-4">
              <Label
                >This initiative was moved from <strong
                  >{RnsStatus[roadblock.status]}</strong
