...
export default function Startups({ startups }: { startups: Startup[] }) {
  ...
  const filteredStartups = startups.filter((startup) => {
    if (
      filter !== 'All Startups' &&
      !filterStatusMap[filter]?.includes(startup.qualificationStatus)
    ) {
      return false;
    }
    ...
    <>
      ...
      <div className="mt-3 pb-10 grid grid-cols-4 gap-5">
        {filteredStartups.map((startup) => (
          <Link
            key={startup.id}
            href={`/startups/${startup.id}/${startup.qualificationStatus === QualificationStatus.QUALIFIED ? 'readiness-level' : 'pending'}`}
          >
              ...
              <CardContent className="h-full">
                <div className="flex flex-col h-full p-3 justify-between">
                  <div className="flex items-center gap-3 mb-1 w-full">
                    ...
                    <span
                      className="text-base font-semibold text-white truncate max-w-[120px]"
                      title={startup.name}
                    >
                    ...
                    <span
                      className={`ml-auto border rounded px-2 py-0.5 text-xs font-semibold ${startupStatusMap[startup.qualificationStatus].border} ${startupStatusMap[startup.qualificationStatus].text} ${startupStatusMap[startup.qualificationStatus].bg}`}
                    >
                  ...
                  <div className="flex items-center gap-2 text-xs text-[#b3bed7] mb-1">
                    ...
                    <span className="ml-1 font-bold text-white">
                      {
                        startup.initiatives.filter(
                          (initiative) =>
                            initiative.status === TaskStatus.Completed
                        ).length
                      }{' '}
                      / {startup.initiatives.length}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-[#b3bed7]">
                    <Image
                      ...
                      height={16}
                    />
                    <span className="text-[#b3bed7]">
                      No consultation pending
                    </span>
                  </div>
                </div>
              </CardContent>
